...
**Owner:** Kojo · **Scope:** payments, payouts, escrow/holds, storefront + pay-link charge sites
...
- **P1 — the flip.** Route *all* new charges internal. `OrderSettlement.prepare/2`
  already emits `:internal`; this phase removes the preceding
  `:dropship_split`/`:platform_fee` gateway outcomes from routing so `:internal` (or a
  hold) is the only result. Both charge sites (`CheckoutLive`, `PayLinkLive`) already
  share `persist_payment!/2`. Exit test: every new charge has ledger rows that sum to
  its amount and include a platform-fee row.
