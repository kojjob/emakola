defmodule Emakola.Cart.CartStore do
  ...
  Stores cart items keyed by session_id, one `cart_items` row per
  `(session_id, variant_id)`. Being in Postgres, carts survive deploys and
  are shared across machines, so the app can scale horizontally.

  ## Cart item shape (as returned by `get_cart/1`)
  ...
  def add_item(session_id, item) do
    ...
    Repo.insert_all(CartItem, [row],
      ...
      conflict_target: [:session_id, :variant_id]
    )
  ...
  def update_quantity(session_id, variant_id, quantity) do
    if quantity <= 0 do
      ...
      item_query(session_id, variant_id)
      |> Repo.update_all(
  ...
  def remove_item(session_id, variant_id) do
    item_query(session_id, variant_id) |> Repo.delete_all()
    :ok
  end
  ...
  def clear_cart(session_id) do
    from(c in CartItem, where: c.session_id == ^session_id) |> Repo.delete_all()
    :ok
  end
  ...
  @doc """
  Removes all cart sessions whose most recently touched item is older than
  `max_age_seconds` ago (newest-item semantics: one fresh item keeps the
  whole cart alive). Called periodically by `Emakola.Cart.CartCleanupWorker`.
  """
  ...
  def cleanup_expired(max_age_seconds) do
    ...
    from(c in CartItem, where: c.session_id in subquery(expired_sessions))
    |> Repo.delete_all()
  ...
  defp item_query(session_id, variant_id) do
    from(c in CartItem, where: c.session_id == ^session_id and c.variant_id == ^variant_id)
  end
