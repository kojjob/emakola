defmodule Emakola.Notifications.Emails.OrderEmail do
  ...
  ## Parameters
    ...
    - `store` — Store struct/map with :name, :slug
  ...
  defp confirmation_html(order, customer, store) do
    ...
    line_items_html = build_line_items_html(line_items, order.currency)
  ...
  defp confirmation_text(order, customer, store) do
    line_items = get_line_items(order)
    line_items_text = build_line_items_text(line_items, order.currency)
  ...
  defp build_line_items_html(line_items, currency) when is_list(line_items) do
    Enum.map_join(line_items, "\n", fn item ->
      title = item_field(item, :product_title)
      sku = item_field(item, :variant_sku)
      qty = item_field(item, :quantity)
      unit_price = item_field(item, :unit_price)
      line_total = item_field(item, :line_total)
      ...
      <tr style="border-bottom:1px solid #f3f4f6;">
        ...
        <td style="padding:12px 0;text-align:right;font-size:14px;color:#374151;">#{EmailHelpers.format_money(unit_price, currency)}</td>
        <td style="padding:12px 0;text-align:right;font-size:14px;color:#111827;font-weight:500;">#{EmailHelpers.format_money(line_total, currency)}</td>
      </tr>
      """
    end)
  end
  ...
  defp build_line_items_text(line_items, currency) when is_list(line_items) do
    Enum.map_join(line_items, "\n", fn item ->
      title = item_field(item, :product_title)
      qty = item_field(item, :quantity)
      unit = EmailHelpers.format_money(item_field(item, :unit_price), currency)
      total = EmailHelpers.format_money(item_field(item, :line_total), currency)
  ...
  defp get_line_items(order) do
    cond do
      is_map(order) and Map.has_key?(order, :line_items) -> order.line_items || []
      true -> []
    end
  end
