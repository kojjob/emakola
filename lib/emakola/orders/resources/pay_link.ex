defmodule Emakola.Orders.PayLink do
  ...
  @doc """
  Is this link still payable? Checks status, then expiry.

  Matches on map shape rather than `%__MODULE__{}`: this resource's struct is
  generated by an Ash `__before_compile__` transformer, so it does not exist
  yet when these clauses are expanded. Under MIX_ENV=prod that surfaces as
  `Emakola.Orders.PayLink.__struct__/1 is undefined` and the release build
  fails — while dev, test and CI all compile it fine, so the breakage only
  appears at deploy time.
  """
  def usable?(%{status: :cancelled}), do: {:error, :cancelled}
  def usable?(%{status: :paid}), do: {:error, :consumed}

  def usable?(%{expires_at: %DateTime{} = at}) do
