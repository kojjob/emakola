defmodule Emakola.Themes.Akwaaba.Shared do
  ...
  @doc """
  A photograph, or — with none — a warm tile carrying the piece's initial.

  The tile sits *beneath* the image rather than instead of it, so a real photo
  simply covers it and nothing has to choose between the two.
  """
  ...
  def photo_or_initial(assigns) do
    ...
    <div
      ...
      aria-hidden="true"
    >
      <span class="select-none text-6xl text-[#E0B39C] [font-family:var(--akwaaba-display)]">
        {String.first(@title)}
      </span>
    </div>
    <.optimized_image
