defmodule Emakola.Themes.Bold.Sections.EditorialBanner do
  ...
  Prints the store's own description; the standing line below it is Bold's
  pre-retrofit fallback, kept exactly as it was.
  """
  ...
  @fallback "Curated for those who appreciate the art of well-made things."
  ...
  def render(assigns) do
    ...
    ~H"""
    <section
      :if={Shared.section_enabled?(@theme, :editorial_banner)}
      class="bg-[#F59E0B]"
    >
  ...
  defp banner_text(_settings, store) do
    if store.description, do: store.description, else: @fallback
  end
end
