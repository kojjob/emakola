defmodule EmakolaWeb.Admin.CampaignLive.Index do
  ...
  def render(assigns) do
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
      <div class="bg-white rounded-2xl border border-slate-200 p-5 hover:shadow-md hover:border-slate-300 transition-all duration-300 cursor-default">
        <div class="flex items-center justify-between mb-4">
          <span class="text-xs font-semibold text-slate-500 uppercase tracking-wider">Total Campaigns</span>
        ...
        <p class="text-xs text-slate-400 mt-2">
          {count_by_status(@campaigns, :active)} active, {count_by_status(@campaigns, :scheduled)} scheduled, {count_by_status(@campaigns, :draft)} drafts
        </p>
      </div>

      <div class="bg-white rounded-2xl border border-slate-200 p-5 hover:shadow-md hover:border-slate-300 transition-all duration-300 cursor-default">
        <div class="flex items-center justify-between mb-4">
          <span class="text-xs font-semibold text-slate-500 uppercase tracking-wider">Messages Sent</span>
      ...
      <div class="bg-white rounded-2xl border border-slate-200 p-5 hover:shadow-md hover:border-slate-300 transition-all duration-300 cursor-default">
        <div class="flex items-center justify-between mb-4">
          <span class="text-xs font-semibold text-slate-500 uppercase tracking-wider">Avg. Open Rate</span>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-8">
      <div :for={campaign <- @campaigns} class="bg-white rounded-2xl border border-slate-200 p-6 hover:shadow-md hover:border-slate-300 transition-all duration-300 relative">
        <div class="flex items-start justify-between mb-4">
          ...
          <div class="flex items-center gap-2">
            ...
            <button
              phx-click={JS.push("confirm_delete", value: %{id: campaign.id}) |> show_modal("delete-campaign-modal")}
        ...
        <div class={"grid #{if campaign.status in [:active], do: "grid-cols-3", else: "grid-cols-2"} gap-3 mb-4"}>
          <div :for={stat <- campaign.stats} class={"text-center py-2.5 #{stat.bg} rounded-xl"}>
            ...
            <p class="text-[10px] text-slate-400 font-medium uppercase tracking-wider">{stat.label}</p>
          </div>
        </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-200 p-6 mb-8">
      <div class="flex items-center justify-between mb-6">
        ...
        <div class="flex items-center gap-4 text-xs">
          <div class="flex items-center gap-1.5"><span class="w-3 h-3 rounded-sm bg-emerald-900"></span><span class="text-slate-500">Sent</span></div>
          <div class="flex items-center gap-1.5"><span class="w-3 h-3 rounded-sm bg-emerald-500"></span><span class="text-slate-500">Opened</span></div>
          <div class="flex items-center gap-1.5"><span class="w-3 h-3 rounded-sm bg-emerald-200"></span><span class="text-slate-500">Clicked</span></div>
        </div>
      </div>
      <div class="relative" style="height: 260px;">
        <svg viewBox="0 0 720 260" class="w-full h-full" preserveAspectRatio="xMidYMid meet" role="img" aria-label="Bar chart showing campaign performance">
          ...
          <text x="50" y="34" text-anchor="end" class="text-[10px] fill-slate-400" font-family="monospace" font-size="10">500</text>
          <text x="50" y="84" text-anchor="end" class="text-[10px] fill-slate-400" font-family="monospace" font-size="10">375</text>
          <text x="50" y="134" text-anchor="end" class="text-[10px] fill-slate-400" font-family="monospace" font-size="10">250</text>
          <text x="50" y="184" text-anchor="end" class="text-[10px] fill-slate-400" font-family="monospace" font-size="10">125</text>
          ...
          <text x="115" y="242" text-anchor="middle" class="text-[10px] fill-slate-400" font-size="10">Flash Sale</text>
          <text x="220" y="242" text-anchor="middle" class="text-[10px] fill-slate-400" font-size="10">Welcome</text>
          <text x="325" y="242" text-anchor="middle" class="text-[10px] fill-slate-400" font-size="10">Cart Recovery</text>
          <text x="430" y="242" text-anchor="middle" class="text-[10px] fill-slate-400" font-size="10">New Arrivals</text>
          <text x="535" y="242" text-anchor="middle" class="text-[10px] fill-slate-400" font-size="10">Loyalty</text>
          <text x="640" y="242" text-anchor="middle" class="text-[10px] fill-slate-400" font-size="10">Re-stock</text>
        </svg>
      </div>
    </div>
    ...
    <div class="mb-8">
      ...
      <p class="text-xs text-slate-400 mb-4">Launch a campaign in minutes with these proven templates</p>
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div class="bg-white rounded-2xl border border-slate-200 p-5 hover:shadow-md hover:border-slate-300 transition-all duration-300">
          ...
          <p class="text-xs text-slate-500 mb-4">Recover lost sales with automated reminders at 1hr and 24hr intervals</p>
          <button phx-click={JS.push("use_template", value: %{template: "abandoned_cart"}) |> show_modal("create-campaign-modal")} class="w-full inline-flex items-center justify-center gap-2 px-4 py-2 bg-white border border-slate-200 rounded-xl text-sm font-medium text-slate-700 hover:bg-slate-50 transition-colors cursor-pointer">
            Use Template
          </button>
        </div>
        <div class="bg-white rounded-2xl border border-slate-200 p-5 hover:shadow-md hover:border-slate-300 transition-all duration-300">
          ...
          <p class="text-xs text-slate-500 mb-4">Greet new customers with a 3-message onboarding sequence</p>
          <button phx-click={JS.push("use_template", value: %{template: "welcome_series"}) |> show_modal("create-campaign-modal")} class="w-full inline-flex items-center justify-center gap-2 px-4 py-2 bg-white border border-slate-200 rounded-xl text-sm font-medium text-slate-700 hover:bg-slate-50 transition-colors cursor-pointer">
            Use Template
          </button>
        </div>
        <div class="bg-white rounded-2xl border border-slate-200 p-5 hover:shadow-md hover:border-slate-300 transition-all duration-300">
          ...
          <p class="text-xs text-slate-500 mb-4">Announce new products with a teaser, launch, and follow-up flow</p>
          <button phx-click={JS.push("use_template", value: %{template: "product_launch"}) |> show_modal("create-campaign-modal")} class="w-full inline-flex items-center justify-center gap-2 px-4 py-2 bg-white border border-slate-200 rounded-xl text-sm font-medium text-slate-700 hover:bg-slate-50 transition-colors cursor-pointer">
            Use Template
          </button>
        </div>
      </div>
    ...
    <.modal id="create-campaign-modal" title="Create Campaign" size={:md} icon="hero-megaphone" icon_class="text-emerald-600">
      <form phx-submit="save_campaign" id="create-campaign-form" class="space-y-4">
        <div>
          ...
          <input type="text" name="campaign[name]" required placeholder="e.g. Easter Sale Blast" class="w-full px-3 py-2.5 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/30 focus:border-emerald-500" />
        </div>
        <div>
          <label class="block text-sm font-medium text-slate-700 mb-1">Channel</label>
          <select name="campaign[channel]" class="w-full px-3 py-2.5 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/30 focus:border-emerald-500">
        ...
        <div>
          ...
          <textarea name="campaign[description]" rows="3" placeholder="Brief description of the campaign..." class="w-full px-3 py-2.5 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/30 focus:border-emerald-500"></textarea>
        </div>
      </form>
      <:footer>
        <div class="flex items-center justify-end gap-3">
          <button type="button" phx-click={hide_modal("create-campaign-modal")} class="px-4 py-2.5 text-sm font-medium text-slate-700 hover:bg-slate-100 rounded-xl transition-colors cursor-pointer">
            Cancel
          </button>
          <button type="submit" form="create-campaign-form" class="px-4 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-sm font-semibold transition-colors cursor-pointer">
            Create Campaign
          </button>
        </div>
      </:footer>
    ...
    [
      ...
      %{
        ...
        stats: [
          ...
          %{value: "GH\u20B5 12,480", label: "Revenue", bg: "bg-slate-50", color: "text-slate-900"}
        ],
