defmodule EmakolaWeb.Admin.SupplyCatalogLive.IndexComponents do
  ...
  def offer_card(assigns) do
    ...
    >
      <div
        data-role="card-art"
        class="relative h-[156px] bg-primary-soft flex items-center justify-center"
      >
        <.glyph name={:product} class="w-14 h-14 text-primary" stroke_width="1.5" />
        <img
          :if={@image_url}
          src={@image_url}
          alt={@title}
      ...
      <div class="flex items-center gap-2 px-3.5 py-2.5 border-t border-slate-100 bg-surface-subtle">
        ...
        >
          <span class={["block h-full rounded-full", stock_fill(@stock)]} style={stock_width(@stock)}>
          </span>
        </span>
