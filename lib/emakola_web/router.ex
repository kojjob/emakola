defmodule EmakolaWeb.Router do
  ...
  # Social-login (OAuth) request + callback routes for merchants, generated by
  # ash_authentication and bridged into the cookie session by AuthController.
  # Mounted at /oauth (not /auth) to avoid colliding with the custom routes
  # above. Ship-dark: a strategy 404s until its creds are set and the buttons
