...
feature_flags = [
  %{key: "whatsapp_ordering", name: "WhatsApp Ordering", description: "Allow customers to place orders via WhatsApp", enabled: true},
  %{key: "rider_dispatch", name: "Rider Dispatch", description: "Enable motorbike rider dispatch for deliveries", enabled: false, required_plan: "growth"},
  %{key: "multi_currency", name: "Multi-Currency", description: "Support for multiple currencies (GHS, NGN, USD)", enabled: false, required_plan: "enterprise"},
  %{key: "sms_notifications", name: "SMS Notifications", description: "Send SMS order updates to customers", enabled: true},
  %{key: "email_receipts", name: "Email Receipts", description: "Send email receipts after purchase", enabled: true},
  %{key: "storefront_v2", name: "Storefront V2", description: "New high-fidelity storefront design", enabled: true},
  %{key: "product_reviews", name: "Product Reviews", description: "Allow customers to leave product reviews", enabled: false},
  %{key: "inventory_alerts", name: "Inventory Alerts", description: "Low stock alerts via WhatsApp/SMS", enabled: true},
  %{key: "analytics_dashboard", name: "Analytics Dashboard", description: "Enhanced analytics with charts and exports", enabled: true, required_plan: "starter"},
  %{key: "custom_domain", name: "Custom Domain", description: "Use a custom domain for the storefront", enabled: false, required_plan: "starter"}
]
...
merchant1 = Seeds.update!(merchant1, :update_profile, %{
  name: "Kwame Asante",
  phone: "+233244123456",
...
# -- Organisation --
org1 = Seeds.create!(Emakola.Accounts.Organisation, :create, %{
...
# -- Store --
store1 = Seeds.create!(Emakola.Accounts.Store, :create, %{
  name: "Kente Kingdom",
  slug: "kente-kingdom",
  currency: "GHS"
})

store1 = Seeds.update!(store1, :update_settings, %{
  description: "Premium handwoven Kente cloth and modern African fashion from Bonwire, Ashanti Region. Every piece tells a story of heritage and craftsmanship.",
...
cat_kente = Seeds.create!(Emakola.Catalog.Category, :create, %{
  name: "Kente Cloth",
  store_id: store1.id,
...
cat_fashion = Seeds.create!(Emakola.Catalog.Category, :create, %{
  name: "Modern Fashion",
  store_id: store1.id,
...
cat_accessories = Seeds.create!(Emakola.Catalog.Category, :create, %{
  name: "Accessories",
  store_id: store1.id,
...
_cat_mens = Seeds.create!(Emakola.Catalog.Category, :create, %{
...
_cat_womens = Seeds.create!(Emakola.Catalog.Category, :create, %{
...
cat_bags = Seeds.create!(Emakola.Catalog.Category, :create, %{
  name: "Bags & Clutches",
  store_id: store1.id,
...
p1 = Seeds.create!(Emakola.Catalog.Product, :create, %{
  title: "Royal Adweneasa Kente Cloth",
  store_id: store1.id,
  category_id: cat_kente.id,
  description: "A masterpiece of Ashanti weaving. The Adweneasa pattern — meaning 'my ideas are exhausted' — represents the highest level of creativity in Kente design. Handwoven by master weavers in Bonwire using premium silk and cotton threads. Each piece takes 3-4 weeks to complete.",
  tags: ["kente", "handwoven", "premium", "ashanti", "bonwire", "silk"],
  seo_title: "Royal Adweneasa Kente Cloth | Authentic Ashanti Weaving",
  seo_description: "Premium handwoven Adweneasa Kente cloth from Bonwire, Ashanti Region. Authentic master weaver craftsmanship."
})

# Size option for kente
size_type_p1 = Seeds.create!(Emakola.Catalog.OptionType, :create, %{
  name: "Size",
  product_id: p1.id,
  store_id: store1.id,
  position: 0
...
size_6yard = Seeds.create!(Emakola.Catalog.OptionValue, :create, %{
  value: "6 Yards",
...
size_12yard = Seeds.create!(Emakola.Catalog.OptionValue, :create, %{
  value: "12 Yards",
  option_type_id: size_type_p1.id,
  store_id: store1.id,
  position: 1
...
v1a = Seeds.create!(Emakola.Catalog.Variant, :create, %{
  product_id: p1.id,
  store_id: store1.id,
  price: 85_000,
...
v1b = Seeds.create!(Emakola.Catalog.Variant, :create, %{
  product_id: p1.id,
  store_id: store1.id,
  price: 150_000,
...
p2 = Seeds.create!(Emakola.Catalog.Product, :create, %{
  title: "Ewe Kente Graduation Stole",
  store_id: store1.id,
  category_id: cat_kente.id,
  description: "Celebrate your achievement with an authentic Ewe Kente stole. Woven in the Volta Region with vibrant patterns symbolizing wisdom and success. Perfect for graduation ceremonies.",
  tags: ["kente", "graduation", "ewe", "stole", "ceremony"]
})

v2 = Seeds.create!(Emakola.Catalog.Variant, :create, %{
  product_id: p2.id,
  store_id: store1.id,
  price: 25_000,
  sku: "KK-EWE-STOLE",
...
p3 = Seeds.create!(Emakola.Catalog.Product, :create, %{
  title: "Ankara-Kente Fusion Midi Dress",
  store_id: store1.id,
  category_id: cat_fashion.id,
  description: "A stunning blend of Ankara prints and Kente strips. This midi dress features a fitted bodice with Kente-accented sleeves and hem. Made-to-measure for the perfect fit.",
  tags: ["ankara", "kente", "fusion", "dress", "women", "modern"]
})

dress_size = Seeds.create!(Emakola.Catalog.OptionType, :create, %{
  name: "Size",
  product_id: p3.id,
  store_id: store1.id
})
...
fusion_variants =
  for {size, pos} <- [{"S", 0}, {"M", 1}, {"L", 2}, {"XL", 3}] do
    ov = Seeds.create!(Emakola.Catalog.OptionValue, :create, %{
      value: size,
    ...
    v = Seeds.create!(Emakola.Catalog.Variant, :create, %{
      product_id: p3.id,
      store_id: store1.id,
      price: 45_000,
      sku: "KK-FUSION-#{size}",
...
p4 = Seeds.create!(Emakola.Catalog.Product, :create, %{
  title: "Handwoven Kente Clutch Bag",
  store_id: store1.id,
  category_id: cat_bags.id,
  description: "Elegant clutch bag lined with authentic Kente fabric. Features a gold clasp and detachable chain strap. Perfect for weddings, funerals, and special occasions.",
  tags: ["clutch", "bag", "kente", "wedding", "accessories"]
})

v4 = Seeds.create!(Emakola.Catalog.Variant, :create, %{
  product_id: p4.id,
  store_id: store1.id,
  price: 18_000,
...
p5 = Seeds.create!(Emakola.Catalog.Product, :create, %{
  title: "Authentic Dagomba Fugu Smock",
  store_id: store1.id,
  category_id: cat_fashion.id,
  description: "Traditional hand-woven smock from Tamale, Northern Region. The Fugu is worn during festivals, durbars, and important ceremonies. Made from locally grown cotton and dyed with natural indigo.",
  tags: ["fugu", "smock", "northern", "dagomba", "traditional", "men"]
})

fugu_color = Seeds.create!(Emakola.Catalog.OptionType, :create, %{
  name: "Color",
  product_id: p5.id,
  store_id: store1.id
})
...
fugu_variants =
  for {color, price, pos} <- [{"Indigo Blue", 35_000, 0}, {"Natural White", 30_000, 1}, {"Earth Brown", 32_000, 2}] do
    ov = Seeds.create!(Emakola.Catalog.OptionValue, :create, %{
      value: color,
...
_p6 = Seeds.create!(Emakola.Catalog.Product, :create, %{
...
cust1 = Seeds.create!(Emakola.Customers.Customer, :create, %{
...
cust2 = Seeds.create!(Emakola.Customers.Customer, :create, %{
...
cust3 = Seeds.create!(Emakola.Customers.Customer, :create, %{
...
cust4 = Seeds.create!(Emakola.Customers.Customer, :create, %{
...
cust5 = Seeds.create!(Emakola.Customers.Customer, :create, %{
...
order1 = Emakola.Orders.CheckoutService.checkout!(store1.id, [
  %{variant_id: v1a.id, quantity: 1}
], customer_id: cust1.id, notes: "Please wrap in gift paper", delivery_fee: 4_500)
{:ok, order1} = order1

pay1 = Seeds.create!(Emakola.Payments.Payment, :create, %{
  store_id: store1.id,
  order_id: order1.id,
  amount: order1.total,
  currency: "GHS",
...
Seeds.update!(pay1, :mark_success, %{gateway_response: %{"status" => "success", "channel" => "mobile_money", "provider" => "mtn"}})
...
order2 = Emakola.Orders.CheckoutService.checkout!(store1.id, [
  %{variant_id: v2.id, quantity: 2},
  %{variant_id: fugu_indigo.id, quantity: 1}
], customer_id: cust2.id, delivery_fee: 3_000)
{:ok, order2} = order2

pay2 = Seeds.create!(Emakola.Payments.Payment, :create, %{
  store_id: store1.id,
  order_id: order2.id,
  amount: order2.total,
  currency: "GHS",
...
Seeds.update!(pay2, :mark_success, %{gateway_response: %{"status" => "success", "channel" => "mobile_money", "provider" => "vodafone"}})
...
order3 = Emakola.Orders.CheckoutService.checkout!(store1.id, [
  %{variant_id: fusion_m.id, quantity: 1},
  %{variant_id: v4.id, quantity: 1}
], customer_id: cust3.id, delivery_fee: 4_500)
{:ok, order3} = order3

pay3 = Seeds.create!(Emakola.Payments.Payment, :create, %{
  store_id: store1.id,
  order_id: order3.id,
  amount: order3.total,
  currency: "GHS",
...
Seeds.update!(pay3, :mark_success, %{gateway_response: %{"status" => "success", "channel" => "mobile_money", "provider" => "airteltigo"}})
...
order4 = Emakola.Orders.CheckoutService.checkout!(store1.id, [
  %{variant_id: v1b.id, quantity: 1}
], customer_id: cust4.id, notes: "For my wedding ceremony", delivery_fee: 1_500)
{:ok, order4} = order4

_pay4 = Seeds.create!(Emakola.Payments.Payment, :create, %{
  store_id: store1.id,
  order_id: order4.id,
  amount: order4.total,
  currency: "GHS",
...
# Order 5: Cancelled
order5 = Emakola.Orders.CheckoutService.checkout!(store1.id, [
...
merchant2 = Seeds.update!(merchant2, :update_profile, %{
  name: "Adjoa Turkson",
  phone: "+233302987654",
  business_name: "Accra Fresh Market"
})

org2 = Seeds.create!(Emakola.Accounts.Organisation, :create, %{
...
store2 = Seeds.create!(Emakola.Accounts.Store, :create, %{
  name: "Accra Fresh Market",
  slug: "accra-fresh",
  currency: "GHS"
})

store2 = Seeds.update!(store2, :update_settings, %{
  description: "Farm-fresh produce, spices, and Ghanaian groceries delivered to your door in Accra. From shito to groundnuts, we bring the market to you.",
...
# -- Categories --
cat_spices = Seeds.create!(Emakola.Catalog.Category, :create, %{
...
cat_grains = Seeds.create!(Emakola.Catalog.Category, :create, %{
  name: "Grains & Staples",
  store_id: store2.id,
...
cat_sauces = Seeds.create!(Emakola.Catalog.Category, :create, %{
  name: "Sauces & Pastes",
  store_id: store2.id,
...
cat_snacks = Seeds.create!(Emakola.Catalog.Category, :create, %{
  name: "Snacks & Nuts",
  store_id: store2.id,
...
ps1 = Seeds.create!(Emakola.Catalog.Product, :create, %{
  title: "Homemade Shito (Hot Pepper Sauce)",
  store_id: store2.id,
  category_id: cat_sauces.id,
  description: "Award-winning homemade shito made with dried fish, shrimp, and scotch bonnet peppers. No preservatives, no artificial flavors. Made fresh weekly in small batches.",
  tags: ["shito", "pepper", "sauce", "homemade", "ghanaian"]
})

shito_size = Seeds.create!(Emakola.Catalog.OptionType, :create, %{
  name: "Size",
  product_id: ps1.id,
  store_id: store2.id
})
...
shito_variants =
  for {size, price, stock, pos} <- [{"250ml Jar", 2_500, 50, 0}, {"500ml Jar", 4_500, 30, 1}, {"1L Family Size", 8_000, 15, 2}] do
    ov = Seeds.create!(Emakola.Catalog.OptionValue, :create, %{
      value: size,
...
ps2 = Seeds.create!(Emakola.Catalog.Product, :create, %{
  title: "Jollof Rice Spice Mix",
  store_id: store2.id,
  category_id: cat_spices.id,
  description: "The secret to perfect Ghana Jollof! Pre-mixed blend of tomato powder, onion, ginger, bay leaf, nutmeg, and our secret ingredient. Just add rice and protein.",
  tags: ["jollof", "spice", "rice", "seasoning", "cooking"]
})

vs2 = Seeds.create!(Emakola.Catalog.Variant, :create, %{
  product_id: ps2.id,
  store_id: store2.id,
  price: 1_500,
  sku: "AFM-JOLLOF-MIX",
...
ps3 = Seeds.create!(Emakola.Catalog.Product, :create, %{
  title: "Premium Basmati Rice",
  store_id: store2.id,
  category_id: cat_grains.id,
  description: "Long-grain basmati rice perfect for Jollof, fried rice, and plain rice. Imported from India, packed locally.",
  tags: ["rice", "basmati", "grain", "staple"]
})

rice_size = Seeds.create!(Emakola.Catalog.OptionType, :create, %{
  name: "Weight",
  product_id: ps3.id,
  store_id: store2.id
})
...
rice_variants =
  for {weight, price, stock, pos} <- [{"2kg", 3_500, 80, 0}, {"5kg", 7_500, 40, 1}, {"25kg Sack", 32_000, 10, 2}] do
    ov = Seeds.create!(Emakola.Catalog.OptionValue, :create, %{
      value: weight,
...
# Roasted Groundnuts
ps4 = Seeds.create!(Emakola.Catalog.Product, :create, %{
  title: "Nkatse (Roasted Groundnuts)",
  store_id: store2.id,
  category_id: cat_snacks.id,
  description: "Freshly roasted groundnuts (peanuts) from Tamale. Crunchy, salty, and addictive. Perfect with Kobi (ripe plantain) or as a standalone snack.",
  tags: ["groundnuts", "nkatse", "peanuts", "snack", "roasted"]
})

vs4 = Seeds.create!(Emakola.Catalog.Variant, :create, %{
  product_id: ps4.id,
  store_id: store2.id,
  price: 1_200,
  sku: "AFM-NKATSE-500G",
...
ps5 = Seeds.create!(Emakola.Catalog.Product, :create, %{
  title: "Dawadawa (Fermented Locust Beans)",
  store_id: store2.id,
  category_id: cat_spices.id,
  description: "Traditional fermented African locust beans. Essential seasoning for soups like groundnut soup, palm nut soup, and light soup. Adds umami depth.",
  tags: ["dawadawa", "locust-bean", "seasoning", "soup", "traditional"]
})

vs5 = Seeds.create!(Emakola.Catalog.Variant, :create, %{
  product_id: ps5.id,
  store_id: store2.id,
  price: 800,
  sku: "AFM-DAWA-200G",
...
# Plantain Chips
ps6 = Seeds.create!(Emakola.Catalog.Product, :create, %{
  title: "Kelewele-Style Plantain Chips",
  store_id: store2.id,
  category_id: cat_snacks.id,
  description: "Crispy plantain chips seasoned with ginger, chili, and cloves. Inspired by Accra's famous kelewele street food. Made with ripe plantain for that perfect sweet-spicy balance.",
  tags: ["plantain", "chips", "kelewele", "snack", "spicy"]
})

vs6 = Seeds.create!(Emakola.Catalog.Variant, :create, %{
  product_id: ps6.id,
  store_id: store2.id,
  price: 1_500,
  sku: "AFM-CHIPS-150G",
...
cust_a1 = Seeds.create!(Emakola.Customers.Customer, :create, %{
...
cust_a2 = Seeds.create!(Emakola.Customers.Customer, :create, %{
...
cust_a3 = Seeds.create!(Emakola.Customers.Customer, :create, %{
...
ord_a1 = Emakola.Orders.CheckoutService.checkout!(store2.id, [
...
pay_a1 = Seeds.create!(Emakola.Payments.Payment, :create, %{
  store_id: store2.id,
  order_id: ord_a1.id,
  amount: ord_a1.total,
  currency: "GHS",
...
Seeds.update!(pay_a1, :mark_success, %{gateway_response: %{"status" => "success", "channel" => "mobile_money", "provider" => "mtn"}})
...
ord_a2 = Emakola.Orders.CheckoutService.checkout!(store2.id, [
...
pay_a2 = Seeds.create!(Emakola.Payments.Payment, :create, %{
  store_id: store2.id,
  order_id: ord_a2.id,
  amount: ord_a2.total,
  currency: "GHS",
...
Seeds.update!(pay_a2, :mark_success, %{gateway_response: %{"status" => "success", "channel" => "mobile_money", "provider" => "vodafone"}})
Seeds.update!(ord_a2, :confirm, %{})

# Order: Pending (Nana Aba)
ord_a3 = Emakola.Orders.CheckoutService.checkout!(store2.id, [
...
Seeds.create!(Emakola.Notifications.Notification, :create, %{
  ...
  body: "Your subscription has been upgraded to the Growth plan. You now have access to rider dispatch and API features.",
  user_id: user1.id
})
