defmodule EmakolaWeb.Storefront.CustomerSessionControllerTest do
  ...
  describe "GET /@:slug/auth/customer-session with a raw (unsigned) subject" do
    test "does not establish a session", %{conn: conn, store: store, customer: customer} do
      ...
      conn =
        get(
          conn,
          "/@#{store.slug}/auth/customer-session?token=#{URI.encode_www_form(raw_subject)}"
        )

      assert redirected_to(conn) == "/@#{store.slug}/login"
      assert Phoenix.Flash.get(conn.assigns.flash, :error)
      refute get_session(conn, :customer_token)
    end
  end

  describe "GET /@:slug/auth/customer-session with a signed subject" do
    test "establishes a session and redirects to account", %{
      ...
      conn =
        get(
          conn,
          "/@#{store.slug}/auth/customer-session?token=#{URI.encode_www_form(signed)}"
        )
      ...
      assert {:ok, _view, html} = live(conn, "/@#{store.slug}/account")
      assert html =~ customer.name
    end
  end

  describe "GET /@:slug/auth/customer-session with garbage or missing token" do
    test "garbage token redirects to login without session write", %{conn: conn, store: store} do
      conn = get(conn, "/@#{store.slug}/auth/customer-session?token=garbage")
    ...
    test "missing token redirects to login", %{conn: conn, store: store} do
      conn = get(conn, "/@#{store.slug}/auth/customer-session")
