defmodule EmakolaWeb.Platform.StoreLiveTest do
  ...
  describe "featuring standing panel" do
    defp with_standing!(store) do
      standing =
    ...
    test "shows the score and slot, and excludes with a reason", %{conn: conn} do
      {conn, _user, _session} = setup_platform_staff(conn, permissions: [:manage_stores])
      store = Factory.create_store!(%{name: "Standing Shop", slug: "standing-shop"})
      standing = with_standing!(store)

      {:ok, view, _html} = live(conn, "/platform/stores")
      view |> element("#store-#{store.id}") |> render_click()

      assert has_element?(view, "#panel-standing-score", "640")

      view
      |> form("#panel-standing form[phx-submit=directory_exclude]")
      |> render_submit(%{"reason" => "counterfeit complaints"})

      reread = Ash.reload!(standing, authorize?: false)
      assert reread.override_excluded
      assert Ash.get!(Emakola.Stores.Store, store.id, authorize?: false).directory_slot == nil
    end

    test "a blank reason is refused with a flash, and nothing changes", %{conn: conn} do
      {conn, _user, _session} = setup_platform_staff(conn, permissions: [:manage_stores])
      store = Factory.create_store!(%{name: "Strict Shop", slug: "strict-shop"})
      standing = with_standing!(store)
      ...
      refute Ash.reload!(standing, authorize?: false).override_excluded
    end

    test "pinning through the panel moves the cached slot", %{conn: conn} do
      {conn, _user, _session} = setup_platform_staff(conn, permissions: [:manage_stores])
      store = Factory.create_store!(%{name: "Pinnable Shop", slug: "pinnable-shop"})
      _standing = with_standing!(store)

      {:ok, view, _html} = live(conn, "/platform/stores")
      view |> element("#store-#{store.id}") |> render_click()

      view
      |> form("#panel-standing form[phx-submit=directory_pin]")
      |> render_submit(%{"slot" => "rising", "reason" => "launch week"})

      assert Ash.get!(Emakola.Stores.Store, store.id, authorize?: false).directory_slot == :rising
    end

    test "a store the worker has never assessed shows no standing panel", %{conn: conn} do
      {conn, _user, _session} = setup_platform_staff(conn, permissions: [:manage_stores])
      store = Factory.create_store!(%{name: "Unassessed Shop", slug: "unassessed-shop"})

      {:ok, view, _html} = live(conn, "/platform/stores")
      view |> element("#store-#{store.id}") |> render_click()

      refute has_element?(view, "#panel-standing")
    end
  end
end
